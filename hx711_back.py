...
class HX711:
    ...
    # if DOUT == 0, hx has data availabe
    def is_ready(self):
        time.sleep(0.0001)   #trying to find out why some bits are being missed
    ...
    def set_gain(self, gain):
        ...
        # this will tell the hx711 to set its internal gain proper
        self.readRawBytes()
    ...
    def readNextBit(self):
       ...
       GPIO.output(self.PD_SCK, True)  # do not hold high for longer than 60us
       #time.sleep(0.00001)   #trying to find out why some bits are being missed
       GPIO.output(self.PD_SCK, False)
       #time.sleep(0.00001)   #trying to find out why some bits are being missed
    ...
    def readRawBytes(self):
        ...
        while not self.is_ready():
           pass
        # may be going to first bit too quickly and not letting HX update
        #time.sleep(0.00001)   #trying to find out why some bits are being missed
        ...
        for x in range(24):
            ...
            time.sleep(0.00001)   #trying to find out why some bits are being missed
            raw_bytes <<= 1
            raw_bytes |= GPIO.input(self.DOUT)
        
        # HX711 Channel and gain factor are set by number of bits read
        # after 24 data bits.
        for i in range(self.GAIN): # gain value = 1 or 2 or 3
    ...
    def read_long(self):
        ...
        #if self.DEBUG_PRINTING:
        #   print(bin(twosComplementValue))
        
        # Convert from 24bit twos-complement to a signed value.
        #signedIntValue = int(self.convertFromTwosComplement24bit(dataBytes))
        #signedIntValue = int(twosComplementValue)
        
        
        #if self.DEBUG_PRINTING:
        #    print(bin(signedIntValue)) 
        # Record the latest sample value we've read.
        # check for bad -1 value that occasioanlly occurs
        #if signedIntValue != -1:
        #self.lastVal = signedIntValue
        if dataBytes < 16777215:  # handle max value spikes
            self.lastVal = dataBytes
        # Return last val in case -1 was found
        return self.lastVal
